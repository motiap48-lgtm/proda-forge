...
const ScheduleGroupComponent: React.FC<ScheduleGroupProps> = ({
  ...
  const [editingCellAbsence, setEditingCellAbsence] = useState<{
    absence: OperatorAbsence;
    operatorName: string;
  } | null>(null);

  // State for creating new absence from empty cell click or drag selection
  const [creatingAbsence, setCreatingAbsence] = useState<{
    operatorId: string;
    operatorName: string;
    date: string;
    endDate?: string;
  ...
  const [unconfirmDialog, setUnconfirmDialog] = useState<{ record: CompensationRecord; operatorName: string } | null>(
    null,
  );
  ...
  const getCompensationRecordsForDate = useCallback(
    (operatorId: string, date: Date): CompensationRecord[] => {
  ...
  const handleConfirmCompensation = useCallback(
    (record: CompensationRecord, e: React.MouseEvent) => {
      e.stopPropagation();
      e.preventDefault();
  ...
  const handleUnconfirmCompensation = useCallback(
    (record: CompensationRecord, operatorName: string, e: React.MouseEvent) => {
      e.stopPropagation();
      e.preventDefault();
  ...
  const calculateCompensationHours = useCallback(
    (operatorId: string): { hours: number; minutes: number } => {
      let totalMinutes = 0;
      days.forEach((day) => {
  ...
  const calculateActualHours = useCallback(
    (operatorId: string): { hours: number; minutes: number } => {
      let totalMinutes = 0;
      days.forEach((day) => {
  ...
  const hasTimesheetData = useCallback(
    (operatorId: string): boolean => {
  ...
  const getOvertimeForDate = useCallback(
    (operatorId: string, date: Date): OvertimeEntry[] => {
      const dateStr = format(date, "yyyy-MM-dd");
      const key = `${operatorId}_${dateStr}`;
  ...
  const hasUnfilledDays = useCallback(
    (operatorId: string): boolean => {
  ...
  const calculateMonthFactHours = useCallback(
    (operatorId: string, month: Date): { hours: number; minutes: number } => {
  ...
  const handleDragSelectionStart = useCallback(
    (operatorId: string, operatorName: string, date: Date, e: React.MouseEvent) => {
  ...
  const isInDragSelection = useCallback(
    (operatorId: string, date: Date): boolean => {
        ...
        <div className="text-left text-sm font-medium text-muted-foreground px-2 py-1.5 bg-muted/50 rounded flex items-center flex-wrap gap-1.5 sm:gap-2 border border-border/40">
          ...
          >
            ...
            <span className="truncate">
              {scheduleName} ({operators.length})
            </span>
            {isCyclicSchedule && (
              <Badge
                variant="outline"
                className="text-[10px] px-1 py-0 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border-amber-300"
              >
      ...
      >
        <div className={cn("overflow-hidden", isCollapsed && "overflow-hidden")}>
          ...
          >
            {/* Employee column */}
            <div
              className="flex-shrink-0 border-r border-border bg-background flex flex-col relative z-50"
            ...
            >
              <div
                ...
                style={{
                  boxShadow:
                    "0 4px 12px -4px hsl(var(--foreground) / 0.15), 0 2px 6px -2px hsl(var(--foreground) / 0.1)",
                }}
              >
              ...
              >
                        ...
                        >
                          ...
                          {!isMobile && (
                            <CompensationPendingIcon
                              ...
                              dateRange={
                                days.length > 0 ? { startDate: days[0], endDate: days[days.length - 1] } : undefined
                              }
                            />
                          )}
                          ...
                          {!isMobile && onEditOperator && (
                            <Button
                              variant="ghost"
                              size="icon"
                              className="h-6 w-6 opacity-50 hover:opacity-100 transition-opacity flex-shrink-0"
                              onClick={(e) => {
                                e.stopPropagation();
                ...
                >
                  <div className={cn("flex items-center flex-wrap", isMobile ? "gap-1" : "gap-2")}>
                    <span className="flex items-center gap-0.5 text-emerald-600" title="Рабочие дни">
                      <CalendarCheck className={cn(isMobile ? "h-2.5 w-2.5" : "h-3 w-3")} />
                      {groupStats.workingDays}
                    </span>
                    <span className="flex items-center gap-0.5 text-rose-500" title="Выходные">
                      <CalendarX className={cn(isMobile ? "h-2.5 w-2.5" : "h-3 w-3")} />
                      {groupStats.offDays}
                    </span>
                    {groupStats.absenceDays > 0 && (
                      <span className="flex items-center gap-0.5 text-orange-500" title="Дни отсутствий">
                        <Plane className={cn(isMobile ? "h-2.5 w-2.5" : "h-3 w-3")} />
                        {groupStats.absenceDays}
                      </span>
                    )}
                  </div>
                </div>
            ...
            >
                  ...
                  {period === "year" ? (
                    ...
                    <>
                      {days.map((day, idx) => {
                        ...
                        const isHoliday =
                          calendarException &&
                          calendarException.exception_type === "holiday" &&
                          !calendarException.is_working_day;
                        const isShortenedDayHeader =
                          calendarException &&
                          calendarException.exception_type === "shortened_day" &&
                          calendarException.is_working_day;
              ...
              {/* Calendar body */}
              <div
                className={cn("pt-1 pb-1 relative z-0", isMobile ? "pl-1 pr-0.5" : "pl-2 pr-0.5")}
                        ...
                        <React.Fragment key={operator.id}>
                          {months.map((month) => {
                            ...
                            return (
                              <div
                                key={month.toISOString()}
                                className={cn(
                                  "text-center p-0.5 h-[var(--sr-row-h)] flex flex-col items-center justify-center rounded-md bg-gradient-to-b from-blue-100 to-blue-200 dark:from-blue-900/30 dark:to-blue-900/50 text-blue-700 dark:text-blue-300 overflow-hidden",
                          ...
                          {(() => {
                            ...
                            return (
                              <div
                                className={cn(
                                  "text-center p-0.5 h-[var(--sr-row-h)] flex flex-col items-center justify-center rounded-md bg-gradient-to-b from-emerald-200 to-emerald-300 dark:from-emerald-800 dark:to-emerald-900 text-emerald-800 dark:text-emerald-200 font-medium overflow-hidden",
                                  isMobile ? "text-[9px]" : "text-xs",
                                )}
                              >
                        ...
                        <>
                          {months.map((month) => {
                            ...
                            return (
                              <div
                                key={month.toISOString()}
                                className="text-center h-8 flex flex-col items-center justify-center text-[10px] text-muted-foreground bg-gradient-to-b from-muted/30 to-muted/50 border-t border-border"
                              >
                                <div>
                        ...
                        <React.Fragment key={operator.id}>
                          {days.map((day) => {
                            ...
                            const effectiveShift =
                              shift || (effectiveIsWorking && !originalIsWorking ? defaultShift : null);
                            const colors = effectiveShift ? shiftColorMap.get(effectiveShift.shift_name) : null;

                            const netMinutes = effectiveShift
                              ? (effectiveShift.net_work_minutes ??
                                effectiveShift.gross_work_minutes - effectiveShift.break_minutes)
                              : 0;
                            const isWeekend = getDay(day) === 0 || getDay(day) === 6;
                            ...
                            const pendingRecords = compensationRecords.filter(
                            ...
                            const approvedOvertimeMinutes = approvedOvertimeRecords.reduce(
                              (sum, r) => sum + (r.duration_minutes || 0),
                              0,
                            );
                            const pendingOvertimeMinutes = pendingOvertimeRecords.reduce(
                              (sum, r) => sum + (r.duration_minutes || 0),
                              0,
                            );
                            ...
                            const holidayException = calendarExceptions.find(
                              (ex) =>
                                ex.exception_date === dateStr && ex.exception_type === "holiday" && !ex.is_working_day,
                            );
                            ...
                            const approvedOvertimeForDay = approvedOvertimeRecords.reduce(
                              (sum, r) => sum + (r.duration_minutes || 0),
                              0,
                            );
                            const confirmedCompensationForDay = confirmedRecords.reduce(
                              (sum, r) => sum + Math.round(r.hours_worked * 60),
                              0,
                            );
                            const factMinutesForDay =
                              (dayTimesheet?.actual_minutes || 0) +
                              approvedOvertimeForDay +
                              confirmedCompensationForDay;
                            const underageMinutes =
                              effectiveIsWorking && hasTimesheetForDay && actualNetMinutes > 0
                            ...
                            if (terminated) {
                              return (
                                <div
                                  key={day.toISOString()}
                                  className={cn(
                                    "text-center p-0.5 h-[var(--sr-row-h)] flex flex-col items-center justify-center rounded-md bg-gradient-to-b from-amber-100 to-amber-200 dark:from-amber-900/30 dark:to-amber-900/50 text-amber-700 dark:text-amber-400 border border-dashed border-amber-400 overflow-hidden",
                                    isMobile ? "text-[9px]" : "text-xs",
                                  )}
                                ...
                                >
                                  ...
                                  <span className={cn(isMobile ? "text-[7px]" : "text-[8px]", "mt-0.5 opacity-75")}>
                                    Увол.
                                  </span>
                                </div>
                              );
                            ...
                            if (beforeHire) {
                              return (
                                <div
                                  key={day.toISOString()}
                                  className={cn(
                                    "text-center p-0.5 h-[var(--sr-row-h)] flex flex-col items-center justify-center rounded-md bg-gradient-to-b from-gray-200 to-gray-300 dark:from-gray-800 dark:to-gray-900 text-gray-400 dark:text-gray-500 overflow-hidden",
                                    isMobile ? "text-[9px]" : "text-xs",
                                  )}
                            ...
                            if (absence) {
                              ...
                              const AbsenceIcon =
                                absence.absence_type === "sick_leave"
                                  ? Stethoscope
                                  : absence.absence_type === "business_trip"
                                    ? Briefcase
                                    : absence.absence_type === "administrative_leave_with_compensation" ||
                                        absence.absence_type === "administrative_leave_without_compensation"
                                      ? FileText
                                      : absence.absence_type === "unauthorized_absence"
                                        ? Ban
                                        : Plane;
                              ...
                              return (
                                <div
                                  ...
                                  className={cn(
                                    ...
                                    absence.absence_type === "annual_leave" &&
                                      "bg-gradient-to-b from-blue-200 to-blue-300 dark:from-blue-900/50 dark:to-blue-900/70 text-blue-700 dark:text-blue-300",
                                    absence.absence_type === "sick_leave" &&
                                      "bg-gradient-to-b from-red-200 to-red-300 dark:from-red-900/50 dark:to-red-900/70 text-red-700 dark:text-red-300",
                                    absence.absence_type === "administrative_leave_with_compensation" &&
                                      "bg-gradient-to-b from-orange-300 to-orange-400 dark:from-orange-800/60 dark:to-orange-900/80 text-orange-800 dark:text-orange-200",
                                    absence.absence_type === "administrative_leave_without_compensation" &&
                                      "bg-gradient-to-b from-orange-200 to-orange-300 dark:from-orange-900/50 dark:to-orange-900/70 text-orange-700 dark:text-orange-300",
                                    absence.absence_type === "maternity_leave" &&
                                      "bg-gradient-to-b from-pink-200 to-pink-300 dark:from-pink-900/50 dark:to-pink-900/70 text-pink-700 dark:text-pink-300",
                                    absence.absence_type === "unpaid_leave" &&
                                      "bg-gradient-to-b from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-800 text-gray-600 dark:text-gray-300",
                                    absence.absence_type === "business_trip" &&
                                      "bg-gradient-to-b from-purple-200 to-purple-300 dark:from-purple-900/50 dark:to-purple-900/70 text-purple-700 dark:text-purple-300",
                                    absence.absence_type === "unauthorized_absence" &&
                                      "bg-gradient-to-b from-rose-300 to-rose-400 dark:from-rose-800/60 dark:to-rose-900/80 text-rose-800 dark:text-rose-200",
                                    absence.absence_type === "other" &&
                                      "bg-gradient-to-b from-slate-200 to-slate-300 dark:from-slate-700 dark:to-slate-800 text-slate-600 dark:text-slate-300",
                                ...
                                >
                                  ...
                                  <GripVertical
                                    className={cn(
                                      "absolute top-0.5 right-0.5 opacity-40 group-hover:opacity-0",
                                  ...
                                  {hasOvertime && (
                                    <Tooltip>
                                      ...
                                      <TooltipContent side="top" className="text-xs">
                                        Переработка: {Math.floor(approvedOvertimeMinutes / 60)}ч{" "}
                                        {approvedOvertimeMinutes % 60}м
                                        {hasPendingOvertime && (
                                          <span className="text-amber-500">
                                            {" "}
                                            (ожидает: {Math.floor(pendingOvertimeMinutes / 60)}ч)
                                          </span>
                                        )}
                                      </TooltipContent>
                                    </Tooltip>
                                  ...
                                  <div
                                    className={cn(
                                      "font-medium truncate w-full px-0.5",
                            ...
                            const handleContextMenu = (e: React.MouseEvent) => {
                              ...
                              if (hasTimesheetForDay) {
                                toast.info(
                                  "На эту дату уже есть записи табеля. Изменение графика и создание отсутствия невозможно.",
                                );
                                return;
                              }
                              ...
                              setEditingOverride({
                                ...
                                shifts:
                                  operator.work_schedules?.work_schedule_shifts?.map((s: any) => ({
                                    shift_number: s.shift_number,
                                    shift_name: s.shift_name,
                                  })) || [],
                                scheduleType: operator.work_schedules?.schedule_type,
                                currentCycleStartDate:
                                  operator.shift_rotation_start_date || operator.work_schedules?.cycle_start_date,
                              });
                            };
                            ...
                            return (
                              <div
                                ...
                                onMouseDown={(e) =>
                                  canCreateAbsence &&
                                  !inPreview &&
                                  handleDragSelectionStart(operator.id, operator.full_name, day, e)
                                }
                                onMouseEnter={() => {
                                  if (isToday(day)) onTodayColumnHover(true);
                                  if (isDraggingSelection && canCreateAbsence)
                                    handleDragSelectionMove(operator.id, day);
                                }}
                                ...
                                className={cn(
                                  ...
                                  canCreateAbsence &&
                                    !inPreview &&
                                    !isDraggingSelection &&
                                    "cursor-pointer hover:ring-2 hover:ring-primary/30 hover:bg-primary/5",
                              ...
                              >
                                    ...
                                    <TooltipContent side="top" className="text-xs max-w-xs">
                                      <div className="space-y-1">
                                        <span className="text-red-500 font-medium block">
                                          Недоработка: -{Math.floor(underageMinutes / 60)}ч
                                          {underageMinutes % 60 > 0 ? ` ${underageMinutes % 60}м` : ""}
                                        </span>
                                        {dayTimesheet?.notes && (
                                ...
                                {hasCompensation &&
                                  (() => {
                                    ...
                                    const confirmedRecords = compensationRecords.filter(
                                        ...
                                        <TooltipTrigger asChild>
                                          ...
                                          >
                                            <Hammer
                                              className={cn(
                                                isMobile ? "h-2 w-2" : "h-2.5 w-2.5",
                                                "transition-colors duration-300 hover:scale-150",
                                                isAnimating
                                            ...
                                            {isAnimating && (
                                              <div className="absolute inset-0 flex items-center justify-center bg-emerald-500 rounded-full animate-scale-in">
                                                <Check
                                                  className={cn(isMobile ? "h-2 w-2" : "h-2.5 w-2.5", "text-white")}
                                                />
                                              </div>
                                            )}
                                          </div>
                                ...
                                {effectiveShift ? (
                                  <div className="w-full text-center flex flex-col items-center">
                                    <div
                                      className="font-medium truncate text-[10px] px-0.5 w-full"
                                      title={effectiveShift.shift_name}
                                    >
                                      ...
                                      >
                                        ...
                                        {hasConfirmedCompensation && (
                                          <span className="text-emerald-600 dark:text-emerald-400">
                                            {" "}
                                            +{Math.round(compensationHoursToday * 100) / 100}ч
                                          </span>
                                        )}
                                        {hasPendingCompensation && (
                                          <span className="text-amber-500 dark:text-amber-400">
                                            {" "}
                                            (~{Math.round(pendingHoursToday * 100) / 100}ч)
                                          </span>
                                        )}
                                      </div>
                                    )}
                                    {cycleInfo && !isMobile && (
                                      <div className="text-[8px] opacity-70 font-semibold whitespace-nowrap">
                                  ...
                                  <div className="flex flex-col items-center">
                                    ...
                                    {cycleInfo && !isMobile && (
                                      <div className="text-[8px] opacity-60 font-semibold whitespace-nowrap">
                                        Д{cycleInfo.dayInCycle}
                                      </div>
                                    )}
                                  </div>
                                    ...
                                    {hasActual ? (
                                      isMobile ? (
                                        <>
                                          ...
                                          <div className="text-[7px] opacity-70 leading-none">
                                            п:{reducedPlanData.hours}ч
                                          </div>
                                        </>
                                      ) : (
                                        <>
                                          <div className="flex items-center gap-0.5">
                                            ...
                                            <span>
                                              {Math.floor(factMinutes / 60)}ч
                                              {factMinutes % 60 > 0 ? ` ${factMinutes % 60}м` : ""}
                                            </span>
                                          </div>
                                          <div className="text-[9px] opacity-80">
                                            п: {reducedPlanData.hours}ч
                                            {reducedPlanData.minutes > 0 ? ` ${reducedPlanData.minutes}м` : ""}
                                          </div>
                                        </>
                                      )
                                    ) : isMobile ? (
                                      ...
                                      <>
                                        ...
                                        {hasOvertimeTotal ? (
                                          <div className="text-[9px] text-purple-700 dark:text-purple-300 font-medium">
                                            +{overtimeHours.hours}ч
                                            {overtimeHours.minutes > 0 ? ` ${overtimeHours.minutes}м` : ""} перераб
                                          </div>
                                        ) : hasCompensationTotal ? (
                                          <div className="text-[9px] text-emerald-600 dark:text-emerald-400 font-medium">
                                            +{compensationHours.hours}ч
                                            {compensationHours.minutes > 0 ? ` ${compensationHours.minutes}м` : ""} отр
                                          </div>
                                        ) : (
                                          reducedPlanData.minutes > 0 && (
                                            <div className="text-[10px] opacity-80">{reducedPlanData.minutes}м</div>
                                          )
                                        )}
                                      </>
                                    )}
                      ...
                      return (
                        <>
                          {days.map((day) => (
                            <div
                              key={day.toISOString()}
                              className="text-center h-8 flex items-center justify-center text-xs text-muted-foreground bg-gradient-to-b from-muted/30 to-muted/50 border-t border-border"
                            >
                              —
                            </div>
                          ))}
                          ...
                          >
                            {groupFactTotal.hasData ? (
                              <>
                                ...
                                <div className="text-[9px] opacity-80">
                                  п: {groupPlanTotal.hours}ч
                                  {groupPlanTotal.minutes > 0 && !isMobile ? ` ${groupPlanTotal.minutes}м` : ""}
                                </div>
                              </>
                            ) : (
                              <>
                                <div>{groupPlanTotal.hours}ч</div>
                                {groupPlanTotal.minutes > 0 && !isMobile && (
                                  <div className="text-[10px]">{groupPlanTotal.minutes}м</div>
                                )}
                              </>
                            )}
                          </div>
            ...
            <AlertDialogDescription>
              {unconfirmDialog && (
                <>
                  ...
                  <strong>
                    {format(new Date(unconfirmDialog.record.compensation_date), "d MMMM yyyy", { locale: ru })}
                  </strong>
                  ?
                  <br />
                  <br />
